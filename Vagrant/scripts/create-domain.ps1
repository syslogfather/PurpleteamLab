...
if ((gwmi win32_computersystem).partofdomain -eq $false) {
  ...
  $nics=Get-WmiObject "Win32_NetworkAdapterConfiguration where IPEnabled='TRUE'" |? { $_.IPAddress[0] -ilike "172.*" }
  foreach($nic in $nics) {
  ...
  foreach($RR in $RRs) {
    if ( (Select-Object  -InputObject $RR HostName,RecordType -ExpandProperty RecordData).IPv4Address -ilike "172.*") {
