Add-Content "c:\windows\system32\drivers\etc\hosts" "        172.16.20.10    ad.childsplay.local"
...
while ($servers_ou_created -ne 1) {
  ...
  try {
    Get-ADOrganizationalUnit -Identity 'OU=Servers,DC=childsplay,DC=local' | Out-Null
    Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) Servers OU already exists. Moving On."
    $servers_ou_created = 1
  }
  catch [Microsoft.ActiveDirectory.Management.ADIdentityNotFoundException] {
    New-ADOrganizationalUnit -Name "Servers" -Server "ad.childsplay.local"
...
Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) Creating Workstations OU..."
  try {
    Get-ADOrganizationalUnit -Identity 'OU=Workstations,DC=childsplay,DC=local' | Out-Null
    Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) Workstations OU already exists. Moving On."
    $workstations_ou_created = 1
  }
  catch [Microsoft.ActiveDirectory.Management.ADIdentityNotFoundException] {
    New-ADOrganizationalUnit -Name "Workstations" -Server "ad.childsplay.local"
